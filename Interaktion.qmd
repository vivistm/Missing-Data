...
for (i in 1:iterations) {
  ...
  model_rf_imp_first <- with(imp_data_rf_imp_first, lm(Y ~ x_1 + x_2 + x_3 + x_4 + x_1*x_3))

  
  pooled_rf_imp_first <- pool(model_rf_imp_first)
...
```{r}
covariates_names <- c("Intercept", "X1", "X2", "X3", "X4", "X1*X3")
