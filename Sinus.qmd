...
sinus_data <- function(seed){ 
  ...
  x_df_org$sinus <- sin(x_df_org[, 1]) # transform
  mu[p+1] <- sin(mu[1])
...
for (i in 1:iterations) {
  ...
  imp_data_rf_imp_first <- mice(data[, -which(names(data) == "sinus")],method = "rf", seed = i, m = n_imp)
  ...
  model_rf_trans_first <-
    with(ImpDataTransFirst, lm(Y ~ x_1 + x_2 + x_3 + x_4 + sin(x_1)))
