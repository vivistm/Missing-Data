...
sinus_data <- function(seed){ 
  ...
  V <- matrix(0.5,p,p)   # correlation of 0.5 betweenn all 6 variables
  ...
  return(list("MAR" = data_sinus_mar, 
              "BD" = data_sinus))
}
```
...
for (i in 1:iterations) {
  ...
  true_beta <- c(1, 1.5, -0.8, 0.5, 0.2,0.4) 
  data <- sinus_data(i) # i macht nichts
  ...
  imp_data_pmm_imp_first <- mice(data[, -which(names(data) == "sinus_x1")], m = n_imp, print = FALSE) 
  ...
  model_pmm_trans_first <-
    with(imp_data_pmm_trans_first, lm(Y ~ x_1 + x_2 + x_3 + x_4 + sinus_x1))
  ...
  imp_data_rf_imp_first <- mice(data[, -which(names(data) == "sinus_x1")],method = "cart", m = n_imp, print = FALSE)
  ...
  model_rf_trans_first <-
    with(imp_data_rf_trans_first, lm(Y ~ x_1 + x_2 + x_3 + x_4 + sinus_x1))
...
zeit_sinus <- toc()
save.image(file='Environment_S.RData')
