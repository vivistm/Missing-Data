...
quadratic_data <- function(seed){
  ...
  # c(intercept, b1,b2,b3,b4,b_quadaric))
...
```{r}
n_imp <- 40
data <- quadratic_data(42)
n_cores<- min(n_imp,availableCores() - 2)
n_cores
Impdata <- futuremice(data[, -which(names(data) == "x1_squared")], m=n_imp,n.core = n_cores,parallelseed = 42)
...
#   ### Idee:
  ...
  imp_data_poly_com <- mice(data,method = meth,pred = pred,quad.outcome ="Y")
