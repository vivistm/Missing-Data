...
```{r}
#| echo: false
pacman::p_load("VIM","rgl","MASS","tidyverse","mice","MASS","future","tictoc")
```
...
n_imp <- 5
n_cores <- 5
#n_cores<- min(n_imp,availableCores()/2)
...
for (i in 1:iterations) {
  ...
  imp_data_pmm_imp_first <- futuremice(data[, -which(names(data) == "x1_squared")], parallelseed = i, m = n_imp, n.cores = n_cores)
...
#   ### Idee:
  ...
  imp_data_poly_com <- futuremice(data,method = meth,pred = pred,quad.outcome ="Y",parallelseed = i, n.core = n_cores)
  ...
  imp_data_rf_imp_first <- futuremice(data[, -which(names(data) == "x1_squared")], method = "rf", parallelseed = i, m = n_imp, n.core = n_cores)
